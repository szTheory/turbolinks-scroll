...
class TurbolinksScroller {
    ...
    setTop() {
        if (this.document.scrollingElement && this.document.scrollingElement.scrollTop) {
    ...
    scrollToTop() {
        if (this.top && this.document.scrollingElement && this.document.scrollingElement.scrollTop) {
    ...
    _setup() {
        const scroller = this;
        delegate_1.delegate("[data-turbolinks-scroll]", "click", "submit", function (event) {
            scroller.setTop();
        }, false);
